      ...
      <b-card class="main-card search-wrapper mb-20">
        ...
        <template v-else>
          <b-row class="mb-2">
            ...
            <b-button variant="primary" class="mr-2 custom-btn-add-common" @click="exportTimetablingStudentResult"
                      style="border: none">
              <font-awesome-icon :icon="['fas','file-excel']"/>
              Xuất danh sách SV theo GV
            </b-button>
          </b-row>

          <b-table
              class="mt-3"
              :items="timetablingStudentResponse.data"
          ...
          <b-row v-if="timetablingStudentResponse.data && timetablingStudentResponse.data.length === 0"
...
import {
  ...
  CREATE_FILE_TIMETABLING_TEACHER, FETCH_TIMETABLING_RESPONSE, FETCH_TIMETABLING_STUDENT_RESPONSE
...
export default {
  ...
  data() {
    return {
      subheading: "Danh sách sinh viên theo giảng viên sau phân công.",
      icon: "pe-7s-portfolio icon-gradient bg-happy-itmeo",
      heading: "Kết quả phân công hướng dẫn",
      loadingHeader: true,
      ...
      fields: [
        ...
        {key: "studentName", label: "Tên sinh viên", visible: true, thStyle: "width: 7%", thClass: 'align-middle'},
  ...
  mounted() {
    ...
    this.fetchTimetablingStudentResponse();
  },
  computed: {
    ...mapGetters(["timetablingStudentResponse"]),
    visibleFields() {
  ...
  methods: {
    ...
    async fetchTimetablingStudentResponse() {
      await this.$store.dispatch(FETCH_TIMETABLING_STUDENT_RESPONSE, {
    ...
    handleReset() {
      this.$router.replace('/admin/timetabling/student/result')
    ...
    checkPermission,
    exportTimetablingStudentResult() {
      this.handleDataFilter()
      this.$store.dispatch(CREATE_FILE_TIMETABLING_STUDENT_RESULT, {
        dataset: this.dataFilter.dataset,
        teacherId: this.dataFilter.teacherId
      });
    },
